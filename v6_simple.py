...
def run_single(lam, seed):
    ...
    mi_v = abs(np.corrcoef(v_mag, h_mag)[0,1]) if len(v_mag)>0 and not np.isnan(np.corrcoef(v_mag, h_mag)[0,1]) else 0
    
    p_mag = np.sqrt(P[:,0]**2 + P[:,1]**2)
    mi_p = abs(np.corrcoef(p_mag, h_mag)[0,1]) if len(p_mag)>0 and not np.isnan(np.corrcoef(p_mag, h_mag)[0,1]) else 0
...
def run():
    lams = [0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06]
    seeds = [42, 123, 456]
    results = []
...
def plot(results):
    ...
    plt.subplot(2,3,3)
    plt.errorbar(lams, [r["mi_v"] for r in results], yerr=[r["mi_v_std"] for r in results], fmt='o-', label='MI(v)', capsize=3)
    plt.errorbar(lams, [r["mi_p"] for r in results], yerr=[r["mi_p_std"] for r in results], fmt='s--', label='MI(p)', capsize=3)
    ...
    plt.tight_layout()
    plt.savefig("fcrs_mis_v6.png", dpi=150)
    print("\nSaved!")
